...
def load_data(database_filepath):
    ...
    Arguments:
        database_filepath -> sqlite database filepath containing clean dataset
    Output:
        X -> a dataframe containing messages
        Y -> array of 36 individual categories columns
        category_names -> Names for the 36 categories
    """
...
def tokenize(text):
    ...
    Arguments:
        text -> messages text to be tokenized
    Output:
...
def build_model():
    ...
    Output:
        A machine learning Pipeline that process text messages and trains a Random Forest classifier.
...
def evaluate_model(model, X_test, y_test, category_names):
    ...
    This function uses the trained model to predict on test dataset and reports the f1 score, precision and recall for each output category of the dataset
...
def save_model(model, model_filepath):
    ...
    This function saves the trained model as Pickle file to the specified filepath.
    
    Arguments:
        model -> the trained model
        pickle_filepath -> the specified filepath to save the modeo as .pkl file
...
def main():
    ...
    This function runs the Machine Learning Pipeline and consists of 4 steps:
        1) load data from SQLite database
        2) Build and fit ML model on training set
        3) evaluate model on test set using f1 score, precision and recall
        4) save trained model to specified filepath as pickle file
