...
def clean_data(df):
    ...
    Output:
        df -> merged messages and categories dataset with categories dataset cleaned
    """
    ...
    df = df[df['related'] != 2]
    #check that all rows with value 2 in related column have been dropped
    np.count_nonzero(df['related']==2, axis=0)
    return df
...
def save_data(df, database_filepath):
    ...
    Arguments:
        df -> merged messages and categories dataset with categories dataset cleaned
        database_filepath -> Path to SQLite database where cleaned data is saved
    """
...
def main():
    """
    Main function for initializing data processing functions. This function consists of three steps:
        1) Load Messages and Categories data and merge them
